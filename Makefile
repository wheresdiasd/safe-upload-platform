.PHONY: lint build secret-scan ci-checks smoke security-smoke test install-hooks help
...
help:
	...
	@echo "  ci-checks       lint + build + secret-scan (run by pre-commit hook and CI)"
	...
	@echo "  test            smoke + security-smoke"
...
ci-checks: lint build secret-scan
